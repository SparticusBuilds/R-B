...
import {
  ...
  BarChart3, RefreshCw, Calculator, Layers, Bookmark,
} from 'lucide-react';
...
const SEED_PRODUCTS = [
  { id: 'p1', name: 'Heptavac P Plus, 250ml', code: 'AH-1029', barcode: '5012345678900', category: 'POM-VPS', price: 68.0, cost: 47.0, supplier: 'MSD Animal Health' },
  { id: 'p2', name: 'Combinex, 5L', code: 'AH-0847', barcode: '5012345678917', category: 'NFA-VPS', price: 58.0, cost: 41.0, supplier: 'Elanco' },
  { id: 'p3', name: 'Footvax, 20-dose', code: 'AH-1103', barcode: '5012345678924', category: 'POM-VPS', price: 74.0, cost: 52.0, supplier: 'MSD Animal Health' },
  { id: 'p4', name: 'Baler twine, 130m', code: 'GEN-0201', barcode: '5012345678931', category: 'General', price: 12.5, cost: 7.8, unitsPerCase: 10, supplier: 'General Farm Supplies' },
  { id: 'p5', name: 'Fencing wire, roll', code: 'GEN-0340', barcode: '5012345678948', category: 'General', price: 28.5, cost: 19.0, unitsPerCase: 1, supplier: 'General Farm Supplies' },
];
...
// --- PIN hashing ---
// PINs are never stored or compared in plaintext. Each staff record holds a random salt and
// the SHA-256 hash of salt+pin — a stolen data export or a glance at browser storage no longer
// hands over anyone's actual employee number. This is still a client-side hash (the JS doing
// the comparison is fully visible to anyone using the browser's devtools), so it does not
// replace real server-side auth — but it closes the "PINs sitting around in plaintext" gap,
// which is the part that matters for a shared shop till and for anyone who opens a data export.
...
async function verifySinglePin(staff, pin) {
  if (!staff || !staff.pinHash || !staff.pinSalt) return false;
  const h = await hashPin(String(pin).trim(), staff.pinSalt);
  return h === staff.pinHash;
}
// Checks a candidate PIN against every staff member and returns the match, or null.
async function verifyStaffPin(staffList, pin) {
  for (const s of staffList) {
    if (await verifySinglePin(s, pin)) return s;
  }
  return null;
}
async function hashedStaffRecord({ id, name, pin }, salt) {
  return { id, name, pinHash: await hashPin(pin, salt), pinSalt: salt };
}
// Seed staff need a stable (not per-load-random) salt so the "is this seed PIN already in the
// user's saved staff list" check on load stays consistent across sessions.
async function hashedSeedStaff() {
  return Promise.all(SEED_STAFF.map((s) => hashedStaffRecord(s, `seed-${s.id}`)));
}
// Migrates any legacy plaintext-pin staff records (from before PIN hashing existed) in place —
// real, already-saved shop data must not be silently discarded when this ships.
async function migrateStaffList(storedStaff) {
  return Promise.all(
    storedStaff.map(async (s) => {
      if (s.pinHash && s.pinSalt) return s;
      const salt = randomSalt();
      return hashedStaffRecord({ id: s.id, name: s.name, pin: s.pin }, salt);
    })
  );
}
...
export default function RBPos() {
  ...
  const [tillCounts, setTillCounts] = useState([]);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const hashedSeeds = await hashedSeedStaff();
        if (res && res.value) {
          const data = JSON.parse(res.value);
          const migratedStaff = await migrateStaffList(data.staffList || []);
          const missingSeedStaff = hashedSeeds.filter((seed) => !migratedStaff.some((s) => s.pinHash === seed.pinHash));
          setProducts(data.products || SEED_PRODUCTS);
          setCustomers(data.customers || SEED_CUSTOMERS);
          setStaffList(migratedStaff.length ? [...migratedStaff, ...missingSeedStaff] : hashedSeeds);
          setCurrentStaff(data.currentStaff || hashedSeeds[0].name);
          ...
          setStaffList(hashedSeeds);
          setCurrentStaff(hashedSeeds[0].name);
          setStockBatches([
        ...
        setCustomers(SEED_CUSTOMERS);
        try {
          const fallbackSeeds = await hashedSeedStaff();
          setStaffList(fallbackSeeds);
          setCurrentStaff(fallbackSeeds[0].name);
        } catch (e2) {
          console.error('PIN hashing unavailable — staff login will not work', e2);
        }
      }
      setLoaded(true);
    })();
    ...
    (async () => {
      ...
      try {
        await window.storage.set(
          STORAGE_KEY,
          JSON.stringify({ products, customers, staffList, currentStaff, stockBatches, payments, transactions, returns, purchaseOrders, quotes, tillCounts, specialOrders, corrections, printedCorrectionIds, lockAfterMinutes }),
          false
        );
      } catch (e) {
  ...
  }, [products, customers, staffList, currentStaff, stockBatches, payments, transactions, returns, purchaseOrders, quotes, tillCounts, specialOrders, corrections, printedCorrectionIds, lockAfterMinutes, loaded]);
  ...
  // Batches with stock left, oldest expiry first — the same order deductStock defaults to.
  // Used to offer staff a choice when a product has more than one batch on the shelf, so the
  // batch actually recorded against a sale can match the one physically picked, rather than
  // always silently taking the soonest-to-expire one.
  function batchesFor(productId) {
    return stockBatches
  ...
  // preferredBatchId lets a sale start consuming from a specific batch (staff's explicit choice,
  // e.g. matching what was physically picked off the shelf for a POM-VPS line) rather than
  // always defaulting to the soonest-expiry one. If the preferred batch doesn't cover the full
  // quantity, the remainder still falls through to normal FIFO-by-expiry across what's left —
  // so a sale can never be blocked by picking a batch that's short a couple of units.
  function deductStock(productId, qtyNeeded, preferredBatchId) {
    let remaining = qtyNeeded;
    const consumed = [];
    setStockBatches((prev) => {
      const relevant = prev.filter((b) => b.productId === productId);
      const preferred = preferredBatchId ? relevant.find((b) => b.id === preferredBatchId) : null;
      const rest = relevant.filter((b) => b.id !== (preferred && preferred.id));
      const sortedRest = [...rest].sort((a, b) => (a.expiry || '9999').localeCompare(b.expiry || '9999'));
      const order = preferred ? [preferred, ...sortedRest] : sortedRest;
      const takenById = {};
      order.forEach((b) => {
        if (remaining <= 0) return;
        const take = Math.min(b.qty, remaining);
        if (take > 0) {
          takenById[b.id] = (takenById[b.id] || 0) + take;
          consumed.push({ batch: b.batch, qty: take });
          remaining -= take;
        }
      });
      return prev.map((b) => (takenById[b.id] ? { ...b, qty: b.qty - takenById[b.id] } : b));
    });
    return consumed;
  }
  ...
  // Quote refs share the same per-year sequential scheme as sales and orders.
  function nextQuoteRef() {
    const yy = String(new Date().getFullYear()).slice(-2);
    const prefix = `QT-${yy}`;
    const countThisYear = quotes.filter((q) => q.ref && q.ref.startsWith(prefix)).length;
    return `${prefix}${String(countThisYear + 1).padStart(6, '0')}`;
  }

  // Quotes never touch stock or accounts on their own — saving one just persists the
  // price it showed, so it can be found again later instead of vanishing once printed.
  function saveQuote({ customerName, items }) {
    ...
    const quote = {
      id: uid('qt'), ref: nextQuoteRef(), date: todayISO(), time: nowHHMM(), staff: currentStaff,
      customerName: customerName || '', items, total, validUntil, converted: false, convertedTxId: '', convertedTxRef: '',
    };
    setQuotes((prev) => [quote, ...prev]);
    return quote;
  }

  function markQuoteConverted(quoteId, tx) {
    setQuotes((prev) => prev.map((q) => (q.id === quoteId ? { ...q, converted: true, convertedTxId: tx.id, convertedTxRef: tx.ref } : q)));
  }

  function nextSpecialOrderRef() {
    const yy = String(new Date().getFullYear()).slice(-2);
    const prefix = `SO-${yy}`;
    const countThisYear = specialOrders.filter((o) => o.ref && o.ref.startsWith(prefix)).length;
    return `${prefix}${String(countThisYear + 1).padStart(6, '0')}`;
  }

  // A special order's deposit is neither a sale nor a payment against existing account debt —
  // it's money held against goods not yet delivered, so it's kept as its own record rather than
  // pushed through recordSale/recordPayment, which would misrepresent it as one of those. At
  // fulfilment, the full sale is recorded normally (so stock deducts correctly); the deposit
  // already collected is shown to staff as a reminder of what's still owed, not auto-applied.
  function createSpecialOrder({ customerId, items, depositAmount, depositMethod, notes }) {
    const total = items.reduce((s, it) => s + it.qty * it.price, 0);
    const so = {
      id: uid('so'), ref: nextSpecialOrderRef(), date: todayISO(), time: nowHHMM(), staff: currentStaff,
      customerId, items, total, depositAmount: depositAmount || 0, depositMethod: depositMethod || '', notes: notes || '',
      status: 'open', fulfilledTxId: '', fulfilledTxRef: '', cancelReason: '',
    };
    setSpecialOrders((prev) => [so, ...prev]);
    return so;
  }

  function fulfillSpecialOrder(orderId, { items, method, pomAssessment, holdOverrideBy }) {
    const so = specialOrders.find((s) => s.id === orderId);
    if (!so) return null;
    const tx = recordSale({ customerId: so.customerId, items: items || so.items, method, pomAssessment, holdOverrideBy });
    setSpecialOrders((prev) => prev.map((s) => (s.id === orderId ? { ...s, status: 'fulfilled', fulfilledTxId: tx.id, fulfilledTxRef: tx.ref } : s)));
    return tx;
  }

  function cancelSpecialOrder(orderId, reason) {
    setSpecialOrders((prev) => prev.map((s) => (s.id === orderId ? { ...s, status: 'cancelled', cancelReason: reason || '' } : s)));
  }

  function recordSale({ customerId, items, method, discountApprovedBy, pomAssessment, holdOverrideBy }) {
    ...
    const lineDetails = items.map((it) => ({ ...it, discount: it.discount || 0, consumed: deductStock(it.productId, it.qty, it.batchId) }));
    const tx = {
      ...
      pomAssessment: pomAssessment || null,
      holdOverrideBy: holdOverrideBy || '',
    };
  ...
  // Ages the outstanding balance by allocating payments against the oldest unpaid charges
  // first (FIFO, same logic already used for stock/return batch matching), then bucketing
  // whatever's left unpaid on each charge by how long it's been outstanding. This is a
  // read-only view over the existing ledger — it doesn't change how the balance itself
  // is calculated or stored.
  function agingBuckets(customerId) {
    const charges = transactions
      .filter((t) => t.customerId === customerId && t.method === 'account')
      .map((t) => ({ date: t.date, amount: t.total }))
      .sort((a, b) => (a.date < b.date ? -1 : a.date > b.date ? 1 : 0));
    let paymentPool = payments.filter((p) => p.customerId === customerId).reduce((s, p) => s + p.amount, 0);
    const today = todayISO();
    const buckets = { current: 0, d30: 0, d60: 0, d90: 0 };
    for (const c of charges) {
      let outstanding = c.amount;
      if (paymentPool > 0) {
        const applied = Math.min(paymentPool, outstanding);
        outstanding -= applied;
        paymentPool -= applied;
      }
  ...
  // Expected cash is today's cash/cheque sales minus today's cash refunds (return credits go
  // back onto an account, not out of the till, so they're excluded) — a read over the existing
  // ledger, same principle as the aging view: no separate "till balance" number to drift out
  // of sync with the transactions actually recorded.
  function expectedCashFor(dateISO) {
    const sales = transactions.filter((t) => t.date === dateISO && t.method === 'cash').reduce((s, t) => s + t.total, 0);
    const refunds = returns.filter((r) => r.date === dateISO && r.originalMethod === 'cash').reduce((s, r) => s + r.refundAmount, 0);
    return sales - refunds;
  }

  function recordTillCount({ date, counted, note }) {
    const expected = expectedCashFor(date);
    const record = {
      id: uid('tc'), date, time: nowHHMM(), staff: currentStaff,
      expected, counted, variance: counted - expected, note: note || '',
    };
    setTillCounts((prev) => [record, ...prev]);
    return record;
  }
  ...
  // Credit holds get the same reason-required, logged-correction treatment as stock and
  // product edits — putting an account on hold (or lifting one) is exactly the kind of
  // change that should leave a "who, when, why" trail behind it.
  function setCustomerHold(customerId, onHold, reason) {
    const existing = customers.find((c) => c.id === customerId);
    if (!existing) return;
    setCustomers((prev) => prev.map((c) => (c.id === customerId ? { ...c, onHold, holdReason: onHold ? reason : '' } : c)));
    logCorrection({
      entity: 'customer', entityId: customerId, entityLabel: existing.name,
      changes: [{ field: 'onHold', from: !!existing.onHold, to: onHold }],
      reason: reason || '',
    });
  }

  // Payment terms are a note + basis, not an enforced due date engine — for a shop this size
  // "pay after harvest" is a real, commonly-agreed term, not an edge case to design around.
  function setCustomerTerms(customerId, terms) {
    setCustomers((prev) => prev.map((c) => (c.id === customerId ? { ...c, terms } : c)));
  }

  async function addStaffMember(name, pin) {
    const trimmedName = name.trim();
    const trimmedPin = String(pin || '').trim();
    if (!trimmedName || !trimmedPin) return;
    const clash = await verifyStaffPin(staffList, trimmedPin);
    if (clash) { alert('That employee number is already in use.'); return; }
    const salt = randomSalt();
    const staff = await hashedStaffRecord({ id: uid('st'), name: trimmedName, pin: trimmedPin }, salt);
    setStaffList((prev) => [...prev, staff]);
    setCurrentStaff(trimmedName);
  }

  // Sets a new PIN for an existing staff member without needing to know their old one —
  // the whole point of one-way hashing is that nobody, including this app, can look a PIN
  // back up, so "reset" is the only way to recover from a forgotten one.
  async function resetStaffPin(staffId, newPin) {
    const trimmedPin = String(newPin || '').trim();
    if (!trimmedPin) { alert('Enter a new employee number.'); return false; }
    const others = staffList.filter((s) => s.id !== staffId);
    const clash = await verifyStaffPin(others, trimmedPin);
    if (clash) { alert('That employee number is already in use by someone else.'); return false; }
    const salt = randomSalt();
    const pinHash = await hashPin(trimmedPin, salt);
    setStaffList((prev) => prev.map((s) => (s.id === staffId ? { ...s, pinHash, pinSalt: salt } : s)));
    return true;
  }

  async function tryUnlock(pin) {
    const match = await verifyStaffPin(staffList, pin);
  ...
  const needsOrderingList = products.filter((p) => Number(p.reorderPoint) > 0 && stockFor(p.id) <= Number(p.reorderPoint));
  const badgeCounts = {
    labels: pendingLabelCount,
    reorder: needsOrderingList.length,
    inventoryAttention: pendingLabelCount + needsOrderingList.length,
  };
    ...
    <div style={{ fontFamily: 'Inter, sans-serif', background: PARCHMENT, minHeight: '100vh', color: CHARCOAL, fontSize: 14 }}>
      <style>{`
        ...
        @media (max-width: 680px) {
          .rb-tile-grid { grid-template-columns: 1fr !important; }
        }
      `}</style>
      ...
      <main style={
        ...
        {!loaded ? (
          ...
          <HomeTab setTab={setTab} badgeCounts={badgeCounts} />
        ) : tab === 'inventory-menu' ? (
          <InventoryMenuTab setTab={setTab} badgeCounts={badgeCounts} />
        ) : tab === 'accounts-menu' ? (
          <AccountsMenuTab setTab={setTab} />
        ) : tab === 'sales' ? (
          <SalesTab products={products} customers={customers} stockFor={stockFor} batchesFor={batchesFor} recordSale={recordSale} staffList={staffList} currentStaff={currentStaff} />
        ) : tab === 'quote' ? (
          <QuoteTab products={products} customers={customers} stockFor={stockFor} batchesFor={batchesFor} recordSale={recordSale} quotes={quotes} saveQuote={saveQuote} markQuoteConverted={markQuoteConverted} staffList={staffList} currentStaff={currentStaff} />
        ) : tab === 'goodsin' ? (
          ...
          <ReorderTab products={products} stockFor={stockFor} needsOrderingList={needsOrderingList} setTab={setTab} />
        ) : tab === 'reports' ? (
          <ReportsTab products={products} stockFor={stockFor} transactions={transactions} />
        ) : tab === 'customers' ? (
          <CustomersTab customers={customers} addCustomer={addCustomer} editCustomer={editCustomer} getBalance={getBalance} setCustomerHold={setCustomerHold} setCustomerTerms={setCustomerTerms} />
        ) : tab === 'accounts' ? (
          <AccountsTab customers={customers} transactions={transactions} payments={payments} returns={returns} getBalance={getBalance} recordPayment={recordPayment} agingBuckets={agingBuckets} />
        ) : tab === 'billing' ? (
          <BillingTab customers={customers} transactions={transactions} payments={payments} returns={returns} />
        ) : tab === 'till' ? (
          <TillTab transactions={transactions} returns={returns} tillCounts={tillCounts} recordTillCount={recordTillCount} currentStaff={currentStaff} />
        ) : tab === 'special-orders' ? (
          <SpecialOrdersTab
            products={products} customers={customers} stockFor={stockFor} batchesFor={batchesFor}
            specialOrders={specialOrders} createSpecialOrder={createSpecialOrder} fulfillSpecialOrder={fulfillSpecialOrder} cancelSpecialOrder={cancelSpecialOrder}
            staffList={staffList} currentStaff={currentStaff}
          />
        ) : (
          <SettingsTab staffList={staffList} addStaffMember={addStaffMember} resetStaffPin={resetStaffPin} corrections={corrections} exportAllData={exportAllData} lockAfterMinutes={lockAfterMinutes} setLockAfterMinutes={setLockAfterMinutes} />
        )}
      </main>
    </div>
  );
...
const HOME_TILES = [
  ...
  { id: 'special-orders', title: 'Special Orders', desc: 'Pre-orders & deposits', accent: BROWN, icon: Bookmark },
  { id: 'returns', title: 'Returns', desc: 'Process a return', accent: SLATE, icon: RotateCcw },
  { id: 'inventory-menu', title: 'Inventory', desc: 'Stock, products, goods in, labels', accent: WHEAT, badgeKey: 'inventoryAttention', icon: Boxes },
  { id: 'accounts-menu', title: 'Accounts', desc: 'Balances, customers, billing, till', accent: PLUM, icon: Wallet },
...
const INVENTORY_TILES = [
  ...
  { id: 'reorder', title: 'Needs Ordering', desc: 'Below reorder point', accent: WARN, badgeKey: 'reorder', icon: RefreshCw },
  { id: 'reports', title: 'Reports', desc: 'Margin & dead stock', accent: PLUM, icon: BarChart3 },
];
...
const ACCOUNTS_TILES = [
  ...
  { id: 'till', title: 'Till', desc: 'End-of-day cash count', accent: SLATE, icon: Calculator },
];

function TileGrid({ tiles, setTab, badgeCounts, fill, columns }) {
  const cols = columns || 3;
  const counts = badgeCounts || {};
  return (
    <div
      className="rb-tile-grid"
      style={{
        display: 'grid',
    ...
    >
      {tiles.map((t) => {
        const badge = t.badgeKey ? (counts[t.badgeKey] || 0) : 0;
        return (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
...
function HomeTab({ setTab, badgeCounts }) {
  return <TileGrid tiles={HOME_TILES} setTab={setTab} badgeCounts={badgeCounts} fill columns={3} />;
}

function InventoryMenuTab({ setTab, badgeCounts }) {
  return <TileGrid tiles={INVENTORY_TILES} setTab={setTab} badgeCounts={badgeCounts} />;
}

function AccountsMenuTab({ setTab }) {
  return <TileGrid tiles={ACCOUNTS_TILES} setTab={setTab} />;
}
...
function ProductSearch({ products, stockFor, onSelect, placeholder }) {
  ...
  return (
    <div ref={boxRef} style={{ position: 'relative', maxWidth: 460 }}>
      <input
        type="text"
        ...
        style={{ width: '100%', padding: '10px 12px', border: `1px solid ${LINE}`, borderRadius: 3, background: PARCHMENT, fontSize: '0.9rem', boxSizing: 'border-box' }}
      />
...
// Shown inline on a cart/line item only when a product has more than one batch in stock —
// lets staff pick which one to record the sale against instead of always silently taking
// whichever expires soonest. Defaults to that soonest-expiry batch, so doing nothing here
...
function BatchPicker({ batches, value, onChange }) {
  ...
  return (
    <select
      ...
      style={{ marginTop: 4, width: '100%', maxWidth: 300, padding: '5px 6px', border: `1px solid ${LINE}`, borderRadius: 3, fontFamily: 'monospace', fontSize: '0.74rem', background: '#fff' }}
    >
      {batches.map((b) => (
        <option key={b.id} value={b.id}>
          batch {b.batch}{b.expiry ? ` · exp ${b.expiry}` : ''} · {b.qty} in stock
        </option>
      ))}
    </select>
  );
...
function SalesTab({ products, customers, stockFor, batchesFor, recordSale, staffList, currentStaff }) {
  const [customerId, setCustomerId] = useState(customers[0].id);
  const [cart, setCart] = useState([]);
  const [receipt, setReceipt] = useState(null);
  const [checkoutMethod, setCheckoutMethod] = useState(null);
  const [checkoutStage, setCheckoutStage] = useState(null); // 'hold' | 'pom' | 'discount' | null
  const [pomAssessment, setPomAssessment] = useState(null);
  const [holdOverrideBy, setHoldOverrideBy] = useState('');
  ...
  const pomItems = cart.filter((it) => {
    const p = products.find((pp) => pp.id === it.productId);
    return p && p.category === 'POM-VPS';
  });
  const sdsItems = cart
    .map((it) => products.find((pp) => pp.id === it.productId))
    .filter((p) => p && p.sdsUrl);
  ...
  function handleSelect(p) {
    ...
    if (existing) {
      ...
      // Default to the soonest-expiry batch (same as deductStock's own default) so behaviour
      // is unchanged unless staff actively pick a different one via the picker on the line.
      const batches = batchesFor(p.id);
      const batchId = batches.length > 0 ? batches[0].id : undefined;
      setCart([...cart, { productId: p.id, name: p.name, price: p.price, qty: 1, discount: 0, batchId }]);
    }
  }

  function setBatch(productId, batchId) {
    setCart(cart.map((it) => (it.productId === productId ? { ...it, batchId } : it)));
  }
  ...
  function checkout(method) {
    ...
    if (method === 'account' && customer.onHold && !holdOverrideBy) {
      setCheckoutStage('hold');
      return;
    }
    if (pomItems.length > 0 && !pomAssessment) {
      setCheckoutStage('pom');
      return;
    }
    if (hasDiscount) {
      setCheckoutStage('discount');
      return;
    }
    finalizeCheckout(method, undefined, pomAssessment);
  }

  function confirmHoldOverride(staffName) {
    setHoldOverrideBy(staffName);
    if (pomItems.length > 0 && !pomAssessment) {
      setCheckoutStage('pom');
      return;
    }
    ...
    finalizeCheckout(checkoutMethod, undefined, pomAssessment, staffName);
  }

  function confirmPomAssessment(assessment) {
    setPomAssessment(assessment);
    if (hasDiscount) {
      setCheckoutStage('discount');
      return;
    }
    finalizeCheckout(checkoutMethod, undefined, assessment);
  }

  function finalizeCheckout(method, discountApprovedBy, assessment, holdOverride) {
    const tx = recordSale({
      customerId, items: cart, method, discountApprovedBy,
      pomAssessment: assessment !== undefined ? assessment : pomAssessment,
      holdOverrideBy: holdOverride !== undefined ? holdOverride : holdOverrideBy,
    });
    setReceipt(tx);
    setCart([]);
    setCheckoutStage(null);
    setCheckoutMethod(null);
    setPomAssessment(null);
    setHoldOverrideBy('');
  }
  ...
  if (receipt) {
    return (
      <Card>
        ...
        <PomAssessmentSummary assessment={receipt.pomAssessment} />
        <div style={{ fontSize: '0.72rem', color: '#8a8577', marginTop: 12, marginBottom: 12 }}>Keep this reference — it's what's needed to process a return.</div>
  ...
  if (checkoutStage === 'hold') {
    return (
      <HoldOverrideGate
        customer={customer} staffList={staffList}
        onCancel={() => { setCheckoutStage(null); setCheckoutMethod(null); }}
        onConfirm={confirmHoldOverride}
      />
    );
  }

  if (checkoutStage === 'pom') {
    return (
      <PomAssessmentGate
        items={pomItems} staffList={staffList} currentStaff={currentStaff}
        onCancel={() => { setCheckoutStage(null); setCheckoutMethod(null); }}
        onConfirm={confirmPomAssessment}
      />
    );
  }

  if (checkoutStage === 'discount') {
    return (
      <DiscountConfirm
        staffList={staffList}
        total={total}
        ...
        onCancel={() => { setCheckoutStage(null); setCheckoutMethod(null); }}
        onConfirm={(staffName) => finalizeCheckout(checkoutMethod, staffName)}
      />
    );
  }
  ...
  return (
    <>
      <Card>
        ...
        <select value={customerId} onChange={(e) => { setCustomerId(e.target.value); setHoldOverrideBy(''); }} style={{ width: '100%', maxWidth: 520, boxSizing: 'border-box', padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: PARCHMENT, fontSize: '0.9rem' }}>
          {customers.map((c) => <option key={c.id} value={c.id}>{c.name} — {c.location}{c.onHold ? ' — ON HOLD' : ''}</option>)}
        </select>
        {customer.onHold && (
          <div style={{ marginTop: 8, fontSize: '0.78rem', color: WARN, fontWeight: 600 }}>
            This account is on hold{customer.holdReason ? `: "${customer.holdReason}"` : ''}. Booking to account will need an override.
          </div>
        )}
      </Card>
        ...
        <Card>
          {cart.map((it) => (
            <div key={it.productId} style={{ padding: '10px 0', borderBottom: `1px solid ${LINE}` }}>
              ...
              {(() => {
                const p = products.find((pp) => pp.id === it.productId);
                if (!p || p.category !== 'POM-VPS') return null;
                const batches = batchesFor(it.productId);
                if (batches.length < 2) return null;
                return (
                  <div style={{ paddingLeft: 2, marginTop: 6 }}>
                    <span style={{ fontSize: '0.74rem', color: '#8a8577' }}>Which batch?</span>
                    <BatchPicker batches={batches} value={it.batchId} onChange={(id) => setBatch(it.productId, id)} />
                  </div>
                );
              })()}
            </div>
          ...
          {pomItems.length > 0 && (
            <div style={{ fontSize: '0.76rem', color: WARN, marginTop: 4 }}>
              Includes a POM-VPS product — a VPS assessment record will be asked for at checkout.
            </div>
          )}
          {sdsItems.length > 0 && (
            <div style={{ marginTop: 10, paddingTop: 10, borderTop: `1px solid ${LINE}` }}>
              <div style={{ fontSize: '0.72rem', color: '#8a8577', marginBottom: 4 }}>Safety data sheets:</div>
              {sdsItems.map((p) => (
                <a key={p.id} href={p.sdsUrl} target="_blank" rel="noopener noreferrer" style={{ display: 'block', fontSize: '0.78rem', color: SLATE, marginBottom: 2 }}>
                  {p.name} — SDS ↗
                </a>
              ))}
            </div>
...
// Shared across every PIN-entry screen (lock screen, discount confirm, hold override): after
// a run of wrong guesses, lock out further attempts for a short cooldown instead of letting
// someone sit there guessing employee numbers indefinitely. Resets on any correct entry.
// This is in-memory only (per screen instance, not persisted) — a reasonable prototype-level
// fix, though real abuse-resistance would need this enforced server-side, not in the browser.
const PIN_MAX_ATTEMPTS = 5;
const PIN_LOCKOUT_SECONDS = 30;
function usePinLockout() {
  ...
  const remainingSeconds = isLocked ? Math.ceil(remainingMs / 1000) : 0;
  ...
  return { isLocked, remainingSeconds, attemptsLeft: PIN_MAX_ATTEMPTS - failCount, fail, success };
}
...
function DiscountConfirm({ staffList, total, discountTotal, onCancel, onConfirm }) {
  ...
  async function submit() {
    if (lockout.isLocked || checking) return;
    setChecking(true);
    const match = await verifyStaffPin(staffList, pin);
    setChecking(false);
    if (!match) {
      lockout.fail();
      setError('Number not recognised — try again.');
      setPin('');
      return;
    }
    lockout.success();
  ...
  return (
    <Card>
      ...
      />
      {lockout.isLocked ? (
        <div style={{ fontSize: '0.78rem', color: WARN, marginBottom: 10 }}>Too many wrong attempts — try again in {lockout.remainingSeconds}s.</div>
      ) : error ? (
        <div style={{ fontSize: '0.78rem', color: WARN, marginBottom: 10 }}>{error}{lockout.attemptsLeft <= 2 ? ` (${lockout.attemptsLeft} attempt${lockout.attemptsLeft === 1 ? '' : 's'} left)` : ''}</div>
      ) : null}
      <div style={{ display: 'flex', gap: 10 }}>
        <button onClick={submit} disabled={lockout.isLocked || checking} style={{ background: GREEN, color: PARCHMENT, border: 'none', padding: '10px 20px', borderRadius: 3, fontWeight: 600, cursor: 'pointer' }}>Confirm sale</button>
...
// UK VMD/SQP point-of-sale record for POM-VPS lines — distinct from the batch/expiry data
// captured at receipt. This is the "who assessed what, for which animal, at time of sale"
// record the regulations actually require; it is NOT a US-style licence-number lookup.
function PomAssessmentGate({ items, staffList, currentStaff, onCancel, onConfirm }) {
  const [species, setSpecies] = useState('');
  const [condition, setCondition] = useState('');
  const [sqp, setSqp] = useState(currentStaff);
  const [error, setError] = useState('');

  function submit() {
    if (!species.trim() || !condition.trim()) {
      setError('Species and the condition being treated/prevented are both required.');
      return;
    }
    onConfirm({ species: species.trim(), condition: condition.trim(), sqp, date: todayISO(), time: nowHHMM() });
  }

  return (
    <Card>
      <div style={{ fontFamily: 'Fraunces, serif', fontSize: '1.15rem', color: GREEN, marginBottom: 6 }}>POM-VPS sale — assessment required</div>
      <div style={{ fontSize: '0.82rem', color: '#6b6659', marginBottom: 14 }}>
        This sale includes a prescription-only (VPS) product. Record who it's for and why before completing checkout — this is what the VMD expects to see at audit, separate from the batch record already captured when this stock was received.
      </div>
      <div style={{ fontSize: '0.76rem', color: '#8a8577', marginBottom: 10 }}>
        Products on this sale requiring a record: {items.map((it) => it.name).join(', ')}
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 10, maxWidth: 420 }}>
        <input type="text" placeholder="Species (e.g. sheep, cattle)" value={species} onChange={(e) => setSpecies(e.target.value)} style={{ padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: PARCHMENT }} />
        <input type="text" placeholder="Condition being treated or prevented" value={condition} onChange={(e) => setCondition(e.target.value)} style={{ padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: PARCHMENT }} />
        <div>
          <Label>Assessed by (SQP, or acting under one)</Label>
          <select value={sqp} onChange={(e) => setSqp(e.target.value)} style={{ width: '100%', padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: PARCHMENT }}>
            {staffList.map((s) => <option key={s.id} value={s.name}>{s.name}</option>)}
          </select>
        </div>
      </div>
      {error && <div style={{ fontSize: '0.78rem', color: WARN, marginBottom: 10 }}>{error}</div>}
      <div style={{ display: 'flex', gap: 10 }}>
        <button onClick={submit} style={{ background: GREEN, color: PARCHMENT, border: 'none', padding: '10px 20px', borderRadius: 3, fontWeight: 600, cursor: 'pointer' }}>Confirm &amp; continue</button>
        <button onClick={onCancel} style={{ background: '#fff', color: CHARCOAL, border: `1px solid ${LINE}`, padding: '10px 20px', borderRadius: 3, fontWeight: 600, cursor: 'pointer' }}>Back to cart</button>
      </div>
    </Card>
  );
...
function PomAssessmentSummary({ assessment }) {
  if (!assessment) return null;
  return (
    <div style={{ fontSize: '0.76rem', color: '#6b6659', background: PARCHMENT, border: `1px solid ${LINE}`, borderRadius: 3, padding: '8px 10px', marginTop: 10 }}>
      <b>VPS assessment recorded</b> — {assessment.species}, {assessment.condition}. By {assessment.sqp}, {assessment.date} {assessment.time}.
    </div>
  );
}

// A held account isn't a silent block — someone can still override it, but has to type
// an employee number to do so, the same pattern already used for discount confirmation.
function HoldOverrideGate({ customer, staffList, onCancel, onConfirm }) {
  const [pin, setPin] = useState('');
  const [error, setError] = useState('');
  const [checking, setChecking] = useState(false);
  const lockout = usePinLockout();
  ...
  return (
    <Card style={{ borderColor: WARN }}>
      <div style={{ fontFamily: 'Fraunces, serif', fontSize: '1.15rem', color: WARN, marginBottom: 6 }}>Account on hold</div>
      <div style={{ fontSize: '0.85rem', color: '#6b6659', marginBottom: 4 }}>
        {customer.name} is on hold{customer.holdReason ? `: "${customer.holdReason}"` : '.'}
      </div>
      <div style={{ fontSize: '0.82rem', color: '#6b6659', marginBottom: 14 }}>Enter an employee number to override the hold and continue this sale.</div>
      <input
        type="password" inputMode="numeric" autoFocus
        value={pin}
        disabled={lockout.isLocked}
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        <button onClick={submit} disabled={lockout.isLocked || checking} style={{ background: WARN, color: '#fff', border: 'none', padding: '10px 20px', borderRadius: 3, fontWeight: 600, cursor: 'pointer' }}>Override &amp; continue</button>
        <button onClick={onCancel} style={{ background: '#fff', color: CHARCOAL, border: `1px solid ${LINE}`, padding: '10px 20px', borderRadius: 3, fontWeight: 600, cursor: 'pointer' }}>Back to cart</button>
      </div>
    </Card>
  );
...
function QuoteTab({ products, customers, stockFor, batchesFor, recordSale, quotes, saveQuote, markQuoteConverted, staffList, currentStaff }) {
  ...
  function doSave() {
    if (cart.length === 0) { alert('Add at least one item first.'); return; }
    const q = saveQuote({ customerName, items: cart });
    setSaved(q);
  }
  ...
  return (
    <>
      <div className="no-print" style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        <button
          onClick={() => setView('new')}
          style={{ flex: 1, padding: '9px 14px', borderRadius: 3, fontWeight: 600, cursor: 'pointer', border: `1px solid ${view === 'new' ? GREEN : LINE}`, background: view === 'new' ? GREEN : '#fff', color: view === 'new' ? PARCHMENT : CHARCOAL }}
        >
          New quote
        </button>
        <button
          onClick={() => setView('history')}
          style={{ flex: 1, padding: '9px 14px', borderRadius: 3, fontWeight: 600, cursor: 'pointer', border: `1px solid ${view === 'history' ? GREEN : LINE}`, background: view === 'history' ? GREEN : '#fff', color: view === 'history' ? PARCHMENT : CHARCOAL }}
        >
      ...
      {view === 'history' ? (
        <QuoteHistory
          quotes={quotes} products={products} customers={customers} stockFor={stockFor} batchesFor={batchesFor}
          recordSale={recordSale} markQuoteConverted={markQuoteConverted} staffList={staffList} currentStaff={currentStaff}
        />
      ) : saved ? (
        <Card>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
            <div style={{ fontFamily: 'Fraunces, serif', fontSize: '1.3rem', color: GREEN, marginBottom: 6 }}>Quote saved</div>
            <div style={{ fontFamily: 'monospace', fontSize: '0.85rem', color: GREEN, fontWeight: 700 }}>{saved.ref}</div>
          </div>
          <div style={{ color: '#6b6659', fontSize: '0.88rem', marginBottom: 4 }}>{fmtMoney(saved.total)} — valid until {saved.validUntil}</div>
          <div style={{ fontSize: '0.76rem', color: '#8a8577', marginBottom: 16 }}>Find it again later under "Saved quotes" using this reference.</div>
          <div style={{ display: 'flex', gap: 10 }}>
            <button onClick={() => window.print()} style={{ background: GREEN, color: PARCHMENT, border: 'none', padding: '10px 18px', borderRadius: 3, fontWeight: 600, cursor: 'pointer' }}>Print quote</button>
            <button onClick={newQuote} style={{ background: '#fff', color: CHARCOAL, border: `1px solid ${LINE}`, padding: '10px 18px', borderRadius: 3, fontWeight: 600, cursor: 'pointer' }}>New quote</button>
          </div>
        </Card>
      ) : (
        <>
          <div className="no-print">
            <Card>
              ...
              <div style={{ fontSize: '0.82rem', color: '#6b6659', marginBottom: 12 }}>
                Builds a price, nothing else. Nothing here touches stock levels or account balances until it's saved and converted to a sale.
              </div>
              <input
                type="text" placeholder="Customer / farm name (optional)" value={customerName}
                onChange={(e) => setCustomerName(e.target.value)}
                style={{ width: '100%', maxWidth: 420, padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: PARCHMENT, boxSizing: 'border-box' }}
              />
            </Card>
            ...
            {cart.length > 0 && (
              <Card>
                ...
                <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
                  <button onClick={doSave} style={{ flex: 1, background: GREEN, color: PARCHMENT, border: 'none', padding: '12px 18px', borderRadius: 3, fontWeight: 600, cursor: 'pointer' }}>
                    Save quote
                  </button>
                  <button onClick={() => window.print()} style={{ background: '#fff', color: CHARCOAL, border: `1px solid ${LINE}`, padding: '12px 18px', borderRadius: 3, fontWeight: 600, cursor: 'pointer' }}>
                    Print
                  </button>
                </div>
              </Card>
...
function QuoteHistory({ quotes, products, customers, stockFor, batchesFor, recordSale, markQuoteConverted, staffList, currentStaff }) {
  ...
  const filtered = quotes.filter((q) => {
    ...
    return (q.ref && q.ref.toLowerCase().includes(s))
  ...
  return (
    <>
      <Card>
        <input
          type="text" placeholder="Search by reference, customer, or product…" value={search} onChange={(e) => setSearch(e.target.value)}
          style={{ width: '100%', maxWidth: 460, padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: PARCHMENT, boxSizing: 'border-box' }}
        />
      </Card>

      {filtered.length === 0 && (
        <Card><div style={{ fontSize: '0.85rem', color: '#8a8577' }}>No saved quotes{search.trim() ? ' match that search' : ' yet'}.</div></Card>
      )}

      {filtered.map((q) => (
        <Card key={q.id}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
            <div style={{ fontWeight: 600, fontSize: '0.9rem' }}>{q.customerName || 'No customer name given'}</div>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontFamily: 'monospace', fontSize: '0.78rem', fontWeight: 700, color: GREEN }}>{q.ref}</div>
              <div style={{ fontFamily: 'monospace', fontSize: '0.74rem', color: '#8a8577' }}>{q.date} · valid until {q.validUntil}</div>
            </div>
          </div>
          {q.items.map((it, i) => (
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.82rem', padding: '4px 0', color: '#6b6659' }}>
              <span>{it.qty}× {it.name}</span>
              <span style={{ fontFamily: 'monospace' }}>{fmtMoney(it.qty * it.price)}</span>
            </div>
          ))}
          <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: 8, borderTop: `1px solid ${LINE}`, marginTop: 6, fontWeight: 700 }}>
            <span>Total</span><span style={{ fontFamily: 'monospace' }}>{fmtMoney(q.total)}</span>
          </div>

          {q.converted ? (
            <div style={{ marginTop: 10, fontSize: '0.8rem', color: OK }}>
              Converted to sale {q.convertedTxRef}
            </div>
          ) : convertingId === q.id ? (
            <ConvertQuoteForm
              quote={q} products={products} customers={customers} stockFor={stockFor} batchesFor={batchesFor} recordSale={recordSale}
              staffList={staffList} currentStaff={currentStaff}
              onDone={(tx) => { markQuoteConverted(q.id, tx); setConvertingId(null); }}
              onCancel={() => setConvertingId(null)}
            />
          ) : (
            <button
              onClick={() => setConvertingId(q.id)}
              style={{ marginTop: 10, background: GREEN_MID, color: PARCHMENT, border: 'none', padding: '9px 16px', borderRadius: 3, fontWeight: 600, cursor: 'pointer' }}
            >
              Convert to sale
            </button>
          )}
...
function ConvertQuoteForm({ quote, products, customers, stockFor, batchesFor, recordSale, staffList, currentStaff, onDone, onCancel }) {
  const [customerId, setCustomerId] = useState(customers[0] ? customers[0].id : '');
  const [method, setMethod] = useState('account');
  const [lines, setLines] = useState(() => quote.items.map((it) => {
    const available = stockFor(it.productId);
    const batches = batchesFor(it.productId);
    return { ...it, available, qty: Math.min(it.qty, Math.max(0, available)), batchId: batches.length > 0 ? batches[0].id : undefined };
  }));
  const [stage, setStage] = useState('form'); // 'form' | 'hold' | 'pom'
  const [holdOverrideBy, setHoldOverrideBy] = useState('');

  const pomLines = lines.filter((l) => {
    const p = products.find((pp) => pp.id === l.productId);
    return p && p.category === 'POM-VPS' && l.qty > 0;
  });
  const total = lines.reduce((s, l) => s + l.qty * l.price, 0);

  function setQty(productId, qty) {
    setLines((prev) => prev.map((l) => (l.productId === productId ? { ...l, qty: Math.max(0, Math.min(Number(qty) || 0, l.available)) } : l)));
  }

  function setBatch(productId, batchId) {
    setLines((prev) => prev.map((l) => (l.productId === productId ? { ...l, batchId } : l)));
  }
  ...
  function proceed() {
    ...
    if (usable.length === 0) { alert('Nothing left to sell — every line is out of stock.'); return; }
    if (method === 'account' && selectedCustomer && selectedCustomer.onHold && !holdOverrideBy) { setStage('hold'); return; }
    if (pomLines.length > 0) { setStage('pom'); return; }
    finalize();
  }

  function finalize(pomAssessment, holdOverride) {
    const usable = lines.filter((l) => l.qty > 0).map((l) => ({ productId: l.productId, name: l.name, price: l.price, qty: l.qty, discount: 0, batchId: l.batchId }));
    const tx = recordSale({ customerId, items: usable, method, pomAssessment, holdOverrideBy: holdOverride !== undefined ? holdOverride : holdOverrideBy });
    onDone(tx);
  }
  ...
  if (stage === 'hold') {
    return (
      <HoldOverrideGate
        customer={selectedCustomer} staffList={staffList}
        onCancel={() => setStage('form')}
        onConfirm={(staffName) => {
          setHoldOverrideBy(staffName);
          if (pomLines.length > 0) { setStage('pom'); return; }
          finalize(undefined, staffName);
        }}
      />
    );
  }
  ...
  if (stage === 'pom') {
    return (
      <PomAssessmentGate
        items={pomLines} staffList={staffList} currentStaff={currentStaff}
        onCancel={() => setStage('form')}
        onConfirm={(assessment) => finalize(assessment)}
      />
    );
  }
  ...
  return (
    <div style={{ marginTop: 10, background: PARCHMENT, border: `1px solid ${LINE}`, borderRadius: 3, padding: 12 }}>
      <Label>Convert to sale</Label>
      <select value={customerId} onChange={(e) => { setCustomerId(e.target.value); setHoldOverrideBy(''); }} style={{ width: '100%', maxWidth: 460, boxSizing: 'border-box', padding: '8px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: '#fff', marginBottom: 8 }}>
        {customers.map((c) => <option key={c.id} value={c.id}>{c.name} — {c.location}{c.onHold ? ' — ON HOLD' : ''}</option>)}
      </select>

      {lines.map((l) => {
        const original = quote.items.find((it) => it.productId === l.productId);
        const short = original && l.available < original.qty;
        const product = products.find((pp) => pp.id === l.productId);
        const batches = product && product.category === 'POM-VPS' ? batchesFor(l.productId) : [];
        return (
          <div key={l.productId} style={{ padding: '6px 0' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
              <div style={{ flex: 1, fontSize: '0.84rem' }}>
                {l.name}
                {short && (
                  <div style={{ fontSize: '0.72rem', color: WARN }}>
                    Only {l.available} in stock now (quoted {original.qty}) — quantity capped
                  </div>
                )}
              </div>
              <input
                type="number" min="0" max={l.available} value={l.qty}
                onChange={(e) => setQty(l.productId, e.target.value)}
                style={{ width: 60, padding: '5px 2px', textAlign: 'center', border: `1px solid ${LINE}`, borderRadius: 3, fontFamily: 'monospace', background: '#fff' }}
              />
            </div>
            {batches.length > 1 && (
              <div style={{ marginTop: 4 }}>
                <span style={{ fontSize: '0.74rem', color: '#8a8577' }}>Which batch?</span>
                <BatchPicker batches={batches} value={l.batchId} onChange={(id) => setBatch(l.productId, id)} />
              </div>
            )}
          </div>
        );
      ...
      <div style={{ display: 'flex', gap: 10, marginTop: 10 }}>
        <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.82rem' }}>
          <input type="radio" checked={method === 'account'} onChange={() => setMethod('account')} /> Book to account
        </label>
        <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.82rem' }}>
          <input type="radio" checked={method === 'cash'} onChange={() => setMethod('cash')} /> Cash / cheque
        </label>
      </div>

      <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
        <button onClick={proceed} style={{ background: GREEN, color: PARCHMENT, border: 'none', padding: '9px 16px', borderRadius: 3, fontWeight: 600, cursor: 'pointer' }}>Confirm sale</button>
        <button onClick={onCancel} style={{ background: 'none', border: `1px solid ${LINE}`, padding: '9px 16px', borderRadius: 3, cursor: 'pointer' }}>Cancel</button>
      </div>
    </div>
  );
}

function NewSpecialOrderForm({ products, customers, createSpecialOrder, onDone }) {
  const [customerId, setCustomerId] = useState(customers[0] ? customers[0].id : '');
  const [cart, setCart] = useState([]);
  ...
  function submit() {
    ...
    if (deposit > total) { alert('Deposit can’t be more than the order total.'); return; }
    const so = createSpecialOrder({ customerId, items: cart, depositAmount: deposit, depositMethod: deposit > 0 ? depositMethod : '', notes: notes.trim() });
    onDone(so);
  }
  ...
  return (
    <>
      <Card>
        <Label>Customer</Label>
        <select value={customerId} onChange={(e) => setCustomerId(e.target.value)} style={{ width: '100%', maxWidth: 520, boxSizing: 'border-box', padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: PARCHMENT }}>
      ...
      {cart.length > 0 && (
        <Card>
          ...
          <div style={{ marginTop: 14, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <input type="number" step="0.01" min="0" placeholder="Deposit taken now (optional)" value={depositAmount} onChange={(e) => setDepositAmount(e.target.value)} style={{ flex: 1, minWidth: 160, padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, fontFamily: 'monospace', background: PARCHMENT }} />
            {parseFloat(depositAmount) > 0 && (
              <select value={depositMethod} onChange={(e) => setDepositMethod(e.target.value)} style={{ padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: PARCHMENT }}>
          ...
          <input type="text" placeholder="Notes (optional — e.g. expected delivery)" value={notes} onChange={(e) => setNotes(e.target.value)} style={{ width: '100%', maxWidth: 420, boxSizing: 'border-box', marginTop: 8, padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: PARCHMENT }} />

          <button onClick={submit} style={{ marginTop: 14, width: '100%', background: GREEN, color: PARCHMENT, border: 'none', padding: '12px 18px', borderRadius: 3, fontWeight: 600, cursor: 'pointer' }}>
            Save special order
          </button>
        </Card>
      )}
...
function FulfillSpecialOrderForm({ order, products, stockFor, batchesFor, staffList, currentStaff, fulfillSpecialOrder, onDone, onCancel }) {
  const [method, setMethod] = useState('account');
  ...
  const [stage, setStage] = useState('form'); // 'form' | 'pom'
  ...
  function finalize(pomAssessment) {
    const usable = lines.filter((l) => l.qty > 0).map((l) => ({ productId: l.productId, name: l.name, price: l.price, qty: l.qty, discount: 0, batchId: l.batchId }));
    const tx = fulfillSpecialOrder(order.id, { items: usable, method, pomAssessment });
    onDone(tx);
  }
  ...
  return (
    <div style={{ marginTop: 10, background: PARCHMENT, border: `1px solid ${LINE}`, borderRadius: 3, padding: 12 }}>
      <Label>Fulfil special order</Label>
      {order.depositAmount > 0 && (
        <div style={{ fontSize: '0.8rem', color: OK, marginBottom: 8 }}>
          {fmtMoney(order.depositAmount)} deposit already collected ({order.depositMethod}, {order.date}) — remember to only take the balance.
        </div>
      )}
      {lines.map((l) => {
        const short = l.available < l.qty || l.available < (order.items.find((it) => it.productId === l.productId) || {}).qty;
        const original = order.items.find((it) => it.productId === l.productId);
        const product = products.find((pp) => pp.id === l.productId);
        const batches = product && product.category === 'POM-VPS' ? batchesFor(l.productId) : [];
        return (
          <div key={l.productId} style={{ padding: '6px 0' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
              <div style={{ flex: 1, fontSize: '0.84rem' }}>
                {l.name}
                {original && l.available < original.qty && (
                  <div style={{ fontSize: '0.72rem', color: WARN }}>Only {l.available} in stock now (ordered {original.qty}) — quantity capped</div>
                )}
              </div>
              <input
                type="number" min="0" max={l.available} value={l.qty}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: 8, borderTop: `1px solid ${LINE}`, marginTop: 6, fontWeight: 700 }}>
        <span>Total due now</span><span style={{ fontFamily: 'monospace' }}>{fmtMoney(total)}</span>
      </div>
      <div style={{ display: 'flex', gap: 10, marginTop: 10 }}>
        <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.82rem' }}>
          <input type="radio" checked={method === 'account'} onChange={() => setMethod('account')} /> Book to account
...
function SpecialOrdersTab({ products, customers, stockFor, batchesFor, recordSale, specialOrders, createSpecialOrder, fulfillSpecialOrder, cancelSpecialOrder, staffList, currentStaff }) {
  const [view, setView] = useState('new'); // 'new' | 'history'
  const [saved, setSaved] = useState(null);
  const [search, setSearch] = useState('');
  ...
  const filtered = specialOrders.filter((o) => {
    if (!search.trim()) return true;
    const c = customers.find((cc) => cc.id === o.customerId);
    const q = search.toLowerCase();
    return (o.ref && o.ref.toLowerCase().includes(q))
  ...
  return (
    <>
      <div className="no-print" style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        <button
          onClick={() => { setView('new'); setSaved(null); }}
          style={{ flex: 1, padding: '9px 14px', borderRadius: 3, fontWeight: 600, cursor: 'pointer', border: `1px solid ${view === 'new' ? GREEN : LINE}`, background: view === 'new' ? GREEN : '#fff', color: view === 'new' ? PARCHMENT : CHARCOAL }}
        >
          New special order
        </button>
        <button
          ...
          style={{ flex: 1, padding: '9px 14px', borderRadius: 3, fontWeight: 600, cursor: 'pointer', border: `1px solid ${view === 'history' ? GREEN : LINE}`, background: view === 'history' ? GREEN : '#fff', color: view === 'history' ? PARCHMENT : CHARCOAL }}
        >
          All orders ({specialOrders.length})
        </button>
      </div>
      ...
      {view === 'new' ? (
        saved ? (
          <Card>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
              <div style={{ fontFamily: 'Fraunces, serif', fontSize: '1.3rem', color: GREEN, marginBottom: 6 }}>Special order saved</div>
              <div style={{ fontFamily: 'monospace', fontSize: '0.85rem', color: GREEN, fontWeight: 700 }}>{saved.ref}</div>
            </div>
            <div style={{ color: '#6b6659', fontSize: '0.88rem', marginBottom: 4 }}>
              {fmtMoney(saved.total)} total{saved.depositAmount > 0 ? ` — ${fmtMoney(saved.depositAmount)} deposit taken (${saved.depositMethod})` : ' — no deposit taken'}
            </div>
            <div style={{ fontSize: '0.76rem', color: '#8a8577', marginBottom: 16 }}>Stock isn't touched until this is fulfilled — find it again under "All orders" using this reference.</div>
            <button onClick={() => { setSaved(null); }} style={{ background: GREEN, color: PARCHMENT, border: 'none', padding: '10px 18px', borderRadius: 3, fontWeight: 600, cursor: 'pointer' }}>
              New special order
            </button>
          </Card>
        ) : (
          <NewSpecialOrderForm products={products} customers={customers} createSpecialOrder={createSpecialOrder} onDone={setSaved} />
        )
      ) : (
        <>
          <Card>
            <input
              type="text" placeholder="Search by reference, customer, or product…" value={search} onChange={(e) => setSearch(e.target.value)}
              style={{ width: '100%', maxWidth: 460, boxSizing: 'border-box', padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: PARCHMENT }}
            />
          </Card>

          {filtered.length === 0 && (
            <Card><div style={{ fontSize: '0.85rem', color: '#8a8577' }}>No special orders{search.trim() ? ' match that search' : ' yet'}.</div></Card>
          )}
            ...
            return (
              <Card key={o.id}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontFamily: 'monospace', fontSize: '0.78rem', fontWeight: 700, color: GREEN }}>{o.ref}</div>
                    <div style={{ fontFamily: 'monospace', fontSize: '0.74rem', color: '#8a8577' }}>{o.date}</div>
                  </div>
                </div>
                {o.items.map((it, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.82rem', padding: '4px 0', color: '#6b6659' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: 8, borderTop: `1px solid ${LINE}`, marginTop: 6, fontWeight: 700 }}>
                  <span>Total</span><span style={{ fontFamily: 'monospace' }}>{fmtMoney(o.total)}</span>
                </div>
                {o.depositAmount > 0 && (
                  <div style={{ fontSize: '0.78rem', color: OK, marginTop: 6 }}>{fmtMoney(o.depositAmount)} deposit taken ({o.depositMethod})</div>
                )}
                {o.notes && <div style={{ fontSize: '0.78rem', color: '#8a8577', fontStyle: 'italic', marginTop: 4 }}>"{o.notes}"</div>}

                {o.status === 'fulfilled' ? (
                  <div style={{ marginTop: 10, fontSize: '0.8rem', color: OK }}>Fulfilled as sale {o.fulfilledTxRef}</div>
                ) : o.status === 'cancelled' ? (
                  <div style={{ marginTop: 10, fontSize: '0.8rem', color: '#8a8577' }}>Cancelled{o.cancelReason ? ` — ${o.cancelReason}` : ''}</div>
                ) : fulfillingId === o.id ? (
                  <FulfillSpecialOrderForm
                    order={o} products={products} stockFor={stockFor} batchesFor={batchesFor} staffList={staffList} currentStaff={currentStaff}
                    fulfillSpecialOrder={fulfillSpecialOrder}
                    onDone={() => setFulfillingId(null)}
                    onCancel={() => setFulfillingId(null)}
                  />
                ) : cancelingId === o.id ? (
                  <div style={{ marginTop: 10, display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    <input
                      type="text" placeholder="Why is this cancelled?" value={cancelReason} onChange={(e) => setCancelReason(e.target.value)}
                      style={{ flex: 1, minWidth: 160, maxWidth: 280, padding: '7px 8px', border: `1px solid ${LINE}`, borderRadius: 3, fontSize: '0.8rem', background: PARCHMENT }}
                    />
                    <button
                      onClick={() => { if (!cancelReason.trim()) { alert('Enter a reason.'); return; } cancelSpecialOrder(o.id, cancelReason.trim()); setCancelingId(null); setCancelReason(''); }}
                      style={{ background: WARN, color: '#fff', border: 'none', padding: '7px 14px', borderRadius: 3, fontSize: '0.76rem', fontWeight: 600, cursor: 'pointer' }}
                    >
                      Confirm cancel
                    </button>
                    <button onClick={() => setCancelingId(null)} style={{ background: 'none', border: `1px solid ${LINE}`, padding: '7px 14px', borderRadius: 3, fontSize: '0.76rem', cursor: 'pointer' }}>Back</button>
                  </div>
                ) : (
                  <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                    <button onClick={() => setFulfillingId(o.id)} style={{ background: GREEN_MID, color: PARCHMENT, border: 'none', padding: '9px 16px', borderRadius: 3, fontWeight: 600, cursor: 'pointer' }}>
                      Fulfil
                    </button>
                    <button onClick={() => setCancelingId(o.id)} style={{ background: 'none', border: `1px solid ${LINE}`, padding: '9px 16px', borderRadius: 3, fontWeight: 600, cursor: 'pointer', color: WARN }}>
                      Cancel order
                    </button>
                  </div>
                )}
    ...
    <>
      <Card>
        ...
        <input
          ...
          style={{ width: '100%', maxWidth: 420, boxSizing: 'border-box', padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: PARCHMENT }}
        />
  ...
  return (
    <div style={{ marginTop: 8, display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
      <input
        ...
        style={{ flex: 1, minWidth: 160, maxWidth: 280, padding: '6px 8px', border: `1px solid ${LINE}`, borderRadius: 3, fontSize: '0.8rem', background: PARCHMENT }}
      />
      <button
...
function GoodsInTab({ products, receiveStock, purchaseOrders, receiveAgainstOrder, closeOrderLine, closeRemainingOrder }) {
  ...
  const [receiveAsCase, setReceiveAsCase] = useState(false);
  const [cases, setCases] = useState(1);
  const [unitsPerCaseInput, setUnitsPerCaseInput] = useState(1);
  ...
  const effectiveQty = receiveAsCase ? Math.max(0, Number(cases) || 0) * Math.max(0, Number(unitsPerCaseInput) || 0) : Number(qty) || 0;

  function selectProduct(p) {
    setSelected(p);
    const perCase = Number(p.unitsPerCase) > 1 ? Number(p.unitsPerCase) : 1;
    setUnitsPerCaseInput(perCase);
    setReceiveAsCase(perCase > 1);
    setCases(1);
    setQty(perCase);
  }
  ...
  function submit() {
    ...
    if (!effectiveQty || effectiveQty <= 0) { alert('Enter a quantity.'); return; }
    receiveStock({ productId: selected.id, batch, qty: effectiveQty, expiry });
    setConfirmedList([{ name: selected.name, batch: batch || '—', qty: effectiveQty, expiry }, ...confirmedList]);
    setSelected(null);
    setBatch('');
    setQty(1);
    setReceiveAsCase(false);
    setCases(1);
    setExpiry('');
  }
  ...
  return (
    <>
      <Card>
        ...
        <select value={orderId} onChange={(e) => setOrderId(e.target.value)} style={{ width: '100%', maxWidth: 480, boxSizing: 'border-box', padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: PARCHMENT }}>
        ...
        <>
          <Card style={{ overflow: 'visible' }}>
            ...
            {selected ? (
              ...
              <ProductSearch products={products} onSelect={selectProduct} placeholder="Search product to receive…" />
            )}

            {selected && (
              <div style={{ display: 'flex', gap: 14, marginTop: 4, marginBottom: 4 }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.8rem', cursor: 'pointer' }}>
                  <input type="radio" checked={!receiveAsCase} onChange={() => setReceiveAsCase(false)} />
                  Receive by units
                </label>
                <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.8rem', cursor: 'pointer' }}>
                  <input type="radio" checked={receiveAsCase} onChange={() => setReceiveAsCase(true)} />
                  Receive by cases
                </label>
              </div>
            )}
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
              <input type="text" placeholder="Batch number" value={batch} onChange={(e) => setBatch(e.target.value)} style={{ flex: 1, minWidth: 120, padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, fontFamily: 'monospace', background: PARCHMENT }} />
              {receiveAsCase ? (
                <>
                  <input type="number" min="1" placeholder="Cases" value={cases} onChange={(e) => setCases(e.target.value)} style={{ width: 80, padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, fontFamily: 'monospace', background: PARCHMENT }} />
                  <span style={{ alignSelf: 'center', color: '#8a8577', fontSize: '0.8rem' }}>×</span>
                  <input type="number" min="1" placeholder="Units/case" value={unitsPerCaseInput} onChange={(e) => setUnitsPerCaseInput(e.target.value)} style={{ width: 90, padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, fontFamily: 'monospace', background: PARCHMENT }} />
                </>
              ) : (
              ...
              <input type="date" value={expiry} onChange={(e) => setExpiry(e.target.value)} style={{ flex: 1, minWidth: 130, padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, fontFamily: 'monospace', background: PARCHMENT }} />
            </div>
            {receiveAsCase && (
              <div style={{ fontSize: '0.76rem', color: '#8a8577', marginTop: 6 }}>
                = {effectiveQty} units total
              </div>
            )}
...
function BatchRow({ batch, editStockBatch }) {
  ...
  return (
    <div style={{ background: PARCHMENT, border: `1px solid ${LINE}`, borderRadius: 3, padding: 10, margin: '4px 0' }}>
      ...
      <input type="text" placeholder="Reason for correction (required)" value={reason} onChange={(e) => setReason(e.target.value)} style={{ width: '100%', maxWidth: 420, boxSizing: 'border-box', padding: '6px 8px', border: `1px solid ${LINE}`, borderRadius: 3, background: '#fff', marginBottom: 8, fontSize: '0.8rem' }} />
...
function StockTab({ products, stockBatches, editStockBatch }) {
  ...
  return (
    <>
      <Card>
        <input type="text" placeholder={`Filter ${products.length} products…`} value={filter} onChange={(e) => setFilter(e.target.value)} style={{ width: '100%', maxWidth: 420, boxSizing: 'border-box', padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: PARCHMENT }} />
      </Card>
      {filtered.map((p) => {
...
function ReorderTab({ products, stockFor, needsOrderingList, setTab }) {
  const bySupplier = {};
  needsOrderingList.forEach((p) => {
  ...
  return (
    <>
      <Card>
        <Label>Needs ordering ({needsOrderingList.length})</Label>
        <div style={{ fontSize: '0.82rem', color: '#6b6659' }}>
          Products at or below their reorder point (set per-product under Inventory → Products). Grouped by supplier so it's easy to jump straight into placing the order.
        </div>
      </Card>

      {needsOrderingList.length === 0 && (
        <Card><div style={{ fontSize: '0.85rem', color: '#8a8577' }}>Nothing's below its reorder point right now.</div></Card>
      )}
        ...
        <Card key={supplier}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 8 }}>
            ...
            {supplier !== '(no supplier set)' && (
              <button onClick={() => setTab('order-supplier')} style={{ background: 'none', border: 'none', color: RUST, fontSize: '0.76rem', textDecoration: 'underline', cursor: 'pointer' }}>
                Order from supplier
              </button>
            )}
          </div>
          {bySupplier[supplier].map((p) => (
            <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderTop: `1px solid ${LINE}`, fontSize: '0.85rem' }}>
              <span>{p.name}</span>
              <span style={{ fontFamily: 'monospace', color: WARN }}>{stockFor(p.id)} in stock · reorder at {p.reorderPoint}</span>
            </div>
          ))}
        </Card>
      ))}
...
function ReportsTab({ products, stockFor, transactions }) {
  const [sortBy, setSortBy] = useState('margin'); // 'margin' | 'marginPct'
  const cutoff = (() => {
    const d = new Date();
    d.setDate(d.getDate() - DEAD_STOCK_DAYS);
    return d.toISOString().slice(0, 10);
  })();
  ...
  const withMargin = products
    ...
    .sort((a, b) => (sortBy === 'margin' ? a.margin - b.margin : a.marginPct - b.marginPct));

  const lastSoldByProduct = {};
  transactions.forEach((t) => {
    t.items.forEach((it) => {
      if (!lastSoldByProduct[it.productId] || t.date > lastSoldByProduct[it.productId]) {
        lastSoldByProduct[it.productId] = t.date;
      }
    });
  });
  const deadStock = products
    .filter((p) => stockFor(p.id) > 0)
    .map((p) => ({ ...p, lastSold: lastSoldByProduct[p.id] || null, stock: stockFor(p.id) }))
    .filter((p) => !p.lastSold || p.lastSold < cutoff)
    .sort((a, b) => (a.lastSold || '').localeCompare(b.lastSold || ''));

  return (
    <>
      <Card>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 4 }}>
          <Label>Margin by product ({withMargin.length})</Label>
          <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} style={{ fontSize: '0.76rem', padding: '4px 8px', border: `1px solid ${LINE}`, borderRadius: 3, background: PARCHMENT }}>
            <option value="margin">Sort by £ margin</option>
            <option value="marginPct">Sort by % margin</option>
          </select>
        </div>
        {withMargin.length === 0 ? (
          <div style={{ fontSize: '0.82rem', color: '#8a8577' }}>No products have a cost set yet — add one under Inventory → Products to see margin here.</div>
        ) : (
          <div style={{ maxHeight: 360, overflowY: 'auto' }}>
            {withMargin.map((p) => (
              <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '7px 0', borderBottom: `1px solid ${LINE}`, fontSize: '0.82rem' }}>
                <span>{p.name}</span>
                <span style={{ fontFamily: 'monospace', display: 'flex', gap: 10 }}>
                  <span style={{ color: '#8a8577' }}>{fmtMoney(p.cost)} → {fmtMoney(p.price)}</span>
                  <span style={{ color: p.marginPct < 20 ? WARN : OK, minWidth: 80, textAlign: 'right' }}>{fmtMoney(p.margin)} ({p.marginPct.toFixed(0)}%)</span>
                </span>
              </div>
            ))}
          </div>
      ...
      <Card>
        <Label>Dead stock — no sale in {DEAD_STOCK_DAYS} days ({deadStock.length})</Label>
        {deadStock.length === 0 ? (
          <div style={{ fontSize: '0.82rem', color: '#8a8577' }}>Nothing in stock has gone quiet for {DEAD_STOCK_DAYS} days.</div>
        ) : (
          deadStock.map((p) => (
            <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 0', borderBottom: `1px solid ${LINE}`, fontSize: '0.82rem' }}>
              <span>{p.name}</span>
              <span style={{ fontFamily: 'monospace', color: '#8a8577' }}>{p.stock} in stock · {p.lastSold ? `last sold ${p.lastSold}` : 'never sold'}</span>
            </div>
          ))
        )}
      </Card>
...
function ProductsTab({ products, addProduct, importProducts, editProduct }) {
  ...
  const supplierNames = [...new Set(products.map((p) => p.supplier).filter(Boolean))].sort();
  const isHazardous = category === 'POM-VPS' || category === 'NFA-VPS';

  function resetForm() {
    setName(''); setCode(''); setBarcode(''); setPrice(''); setCost(''); setReorderPoint(''); setSdsUrl(''); setUnitsPerCase(''); setSupplier('');
  }

  function submit() {
    if (!name.trim()) { alert('Enter a product name.'); return; }
    const candidate = {
      name: name.trim(), code: code.trim(), barcode: barcode.trim(), category, price: parseFloat(price) || 0,
      cost: parseFloat(cost) || 0, reorderPoint: parseFloat(reorderPoint) || 0, sdsUrl: sdsUrl.trim(),
      unitsPerCase: parseFloat(unitsPerCase) || 1, supplier: supplier.trim(),
    };
      ...
      complete: (results) => {
        ...
        rows.forEach((r) => {
          const candidate = {
            ...
            unitsPerCase: parseFloat(r.unitsPerCase) || 1,
            supplier: String(r.supplier || '').trim(),
          };
    ...
    <>
      <Card>
        ...
        <div style={{ fontSize: '0.82rem', color: '#6b6659', marginBottom: 10 }}>
          Columns: <span style={{ fontFamily: 'monospace' }}>name, code, barcode, category, price, cost, reorderPoint, sdsUrl, unitsPerCase, supplier</span> — only name is required. cost drives the margin report under Inventory → Reports, reorderPoint feeds Needs Ordering, sdsUrl is offered at Sales checkout for POM-VPS/NFA-VPS lines, unitsPerCase lets Goods In book in a delivery by the case, supplier is needed only if you'll order this item via Inventory → Order from Supplier. Anything spelt similarly to an existing product, sharing a code, or sharing a barcode, gets held for you to check rather than added automatically.
        </div>
      ...
      <Card>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxWidth: 460 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <input type="number" step="0.01" placeholder="Cost (optional)" value={cost} onChange={(e) => setCost(e.target.value)} style={{ width: 100, padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, fontFamily: 'monospace', background: PARCHMENT }} />
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <input type="number" step="1" placeholder="Reorder point (optional)" value={reorderPoint} onChange={(e) => setReorderPoint(e.target.value)} style={{ flex: 1, padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, fontFamily: 'monospace', background: PARCHMENT }} />
            <input type="number" step="1" min="1" placeholder="Units per case (optional)" value={unitsPerCase} onChange={(e) => setUnitsPerCase(e.target.value)} style={{ flex: 1, padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, fontFamily: 'monospace', background: PARCHMENT }} />
          </div>
          {isHazardous && (
            <input
              type="text" placeholder="SDS (Safety Data Sheet) link — shown at checkout" value={sdsUrl} onChange={(e) => setSdsUrl(e.target.value)}
      ...
      <Card>
        ...
        <input type="text" placeholder="Filter…" value={filter} onChange={(e) => setFilter(e.target.value)} style={{ width: '100%', maxWidth: 420, boxSizing: 'border-box', padding: '8px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: PARCHMENT, marginBottom: 10 }} />
...
function ProductRow({ product, editProduct, supplierNames }) {
  ...
  const [unitsPerCase, setUnitsPerCase] = useState(product.unitsPerCase || '');
  const [supplier, setSupplier] = useState(product.supplier || '');
  const [reason, setReason] = useState('');
  const isHazardous = product.category === 'POM-VPS' || product.category === 'NFA-VPS';

  function save() {
    if (!reason.trim()) { alert('Enter a reason for this correction.'); return; }
    editProduct(product.id, {
      name: name.trim(), code: code.trim(), barcode: barcode.trim(), price: parseFloat(price) || 0,
      cost: parseFloat(cost) || 0, reorderPoint: parseFloat(reorderPoint) || 0, sdsUrl: sdsUrl.trim(),
      unitsPerCase: parseFloat(unitsPerCase) || 1, supplier: supplier.trim(),
    }, reason.trim());
    setEditing(false);
  ...
  const margin = product.cost > 0 ? product.price - product.cost : null;
  ...
  if (!editing) {
    return (
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '7px 0', borderBottom: `1px solid ${LINE}`, fontSize: '0.82rem' }}>
        ...
        <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          {marginPct !== null && (
            <span style={{ fontFamily: 'monospace', fontSize: '0.68rem', color: marginPct < 20 ? WARN : '#8a8577' }}>{marginPct.toFixed(0)}% margin</span>
          )}
          {product.sdsUrl && (
            <a href={product.sdsUrl} target="_blank" rel="noopener noreferrer" style={{ fontSize: '0.7rem', color: SLATE }} onClick={(e) => e.stopPropagation()}>SDS ↗</a>
          )}
  ...
  return (
    <div style={{ background: PARCHMENT, border: `1px solid ${LINE}`, borderRadius: 3, padding: 10, margin: '4px 0' }}>
      <div style={{ display: 'flex', gap: 8, marginBottom: 8, flexWrap: 'wrap' }}>
        ...
        <input type="number" step="0.01" value={price} onChange={(e) => setPrice(e.target.value)} placeholder="Price" style={{ width: 80, padding: '6px 8px', border: `1px solid ${LINE}`, borderRadius: 3, fontFamily: 'monospace', background: '#fff' }} />
        <input type="number" step="0.01" value={cost} onChange={(e) => setCost(e.target.value)} placeholder="Cost" style={{ width: 80, padding: '6px 8px', border: `1px solid ${LINE}`, borderRadius: 3, fontFamily: 'monospace', background: '#fff' }} />
        <input type="number" step="1" value={reorderPoint} onChange={(e) => setReorderPoint(e.target.value)} placeholder="Reorder pt" style={{ width: 90, padding: '6px 8px', border: `1px solid ${LINE}`, borderRadius: 3, fontFamily: 'monospace', background: '#fff' }} />
        <input type="number" step="1" min="1" value={unitsPerCase} onChange={(e) => setUnitsPerCase(e.target.value)} placeholder="Units/case" style={{ width: 90, padding: '6px 8px', border: `1px solid ${LINE}`, borderRadius: 3, fontFamily: 'monospace', background: '#fff' }} />
      ...
      {isHazardous && (
        <input
          type="text" value={sdsUrl} onChange={(e) => setSdsUrl(e.target.value)} placeholder="SDS (Safety Data Sheet) link"
          style={{ width: '100%', maxWidth: 420, boxSizing: 'border-box', padding: '6px 8px', border: `1px solid ${LINE}`, borderRadius: 3, background: '#fff', marginBottom: 8 }}
        />
      )}
      <input type="text" placeholder="Reason for correction (required)" value={reason} onChange={(e) => setReason(e.target.value)} style={{ width: '100%', maxWidth: 420, boxSizing: 'border-box', padding: '6px 8px', border: `1px solid ${LINE}`, borderRadius: 3, background: '#fff', marginBottom: 8, fontSize: '0.8rem' }} />
      <div style={{ display: 'flex', gap: 8 }}>
      ...
      {open && (
        <div style={{ marginTop: 10, background: PARCHMENT, border: `1px solid ${LINE}`, borderRadius: 3, padding: 12 }}>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 10 }}>
            ...
            <select value={reason} onChange={(e) => handleReasonChange(e.target.value)} style={{ flex: 1, minWidth: 160, maxWidth: 300, padding: '8px', border: `1px solid ${LINE}`, borderRadius: 3, background: '#fff' }}>
  ...
  return (
    <>
      <Card>
        ...
        <input type="text" placeholder="Sale reference (RB-000123), customer, or product…" value={search} onChange={(e) => setSearch(e.target.value)} style={{ width: '100%', maxWidth: 460, boxSizing: 'border-box', padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: PARCHMENT }} />
      ...
      {filtered.map((tx) => {
        ...
        return (
          <Card key={tx.id}>
            ...
            <PomAssessmentSummary assessment={tx.pomAssessment} />
          </Card>
        );
      })}
    </>
...
function AgingRow({ label, amount, warn }) {
  if (amount <= 0.005) return null;
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.72rem', fontFamily: 'monospace', color: warn ? WARN : '#6b6659' }}>
      <span>{label}</span><span>{fmtMoney(amount)}</span>
    </div>
  );
}

function AccountRow({ customer, transactions, payments, returns, getBalance, recordPayment, agingBuckets }) {
  ...
  const aging = agingBuckets(customer.id);
    ...
    <Card>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8, flexWrap: 'wrap', gap: 10 }}>
        ...
        <div style={{ textAlign: 'right' }}>
          ...
          {bal > 0.005 && (
            <div style={{ marginTop: 4 }}>
              <AgingRow label="Current (0–30d)" amount={aging.current} />
              <AgingRow label="31–60d" amount={aging.d30} warn />
              <AgingRow label="61–90d" amount={aging.d60} warn />
              <AgingRow label="90d+" amount={aging.d90} warn />
            </div>
          )}
      ...
      {showForm && (
        <div style={{ background: PARCHMENT, border: `1px solid ${LINE}`, borderRadius: 3, padding: 12, marginBottom: 10 }}>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            <input type="text" placeholder="Note (optional)" value={note} onChange={(e) => setNote(e.target.value)} style={{ flex: 1, minWidth: 120, maxWidth: 280, padding: '8px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: '#fff' }} />
          </div>
...
function AccountsTab({ customers, transactions, payments, returns, getBalance, recordPayment, agingBuckets }) {
  const totals = customers.reduce(
    (acc, c) => {
      const a = agingBuckets(c.id);
      acc.current += a.current; acc.d30 += a.d30; acc.d60 += a.d60; acc.d90 += a.d90;
      return acc;
    },
    { current: 0, d30: 0, d60: 0, d90: 0 }
  );
  const totalOwed = totals.current + totals.d30 + totals.d60 + totals.d90;

  return (
    <>
      {totalOwed > 0.005 && (
        <Card>
          <Label>Aged debt across all accounts</Label>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginTop: 8 }}>
            <div>
              <div style={{ fontSize: '0.68rem', color: '#8a8577', textTransform: 'uppercase' }}>Current</div>
              <div style={{ fontFamily: 'monospace', fontWeight: 700 }}>{fmtMoney(totals.current)}</div>
            </div>
            <div>
              <div style={{ fontSize: '0.68rem', color: '#8a8577', textTransform: 'uppercase' }}>31–60d</div>
              <div style={{ fontFamily: 'monospace', fontWeight: 700, color: totals.d30 > 0 ? WARN : CHARCOAL }}>{fmtMoney(totals.d30)}</div>
            </div>
            <div>
              <div style={{ fontSize: '0.68rem', color: '#8a8577', textTransform: 'uppercase' }}>61–90d</div>
              <div style={{ fontFamily: 'monospace', fontWeight: 700, color: totals.d60 > 0 ? WARN : CHARCOAL }}>{fmtMoney(totals.d60)}</div>
            </div>
            <div>
              <div style={{ fontSize: '0.68rem', color: '#8a8577', textTransform: 'uppercase' }}>90d+</div>
              <div style={{ fontFamily: 'monospace', fontWeight: 700, color: totals.d90 > 0 ? WARN : CHARCOAL }}>{fmtMoney(totals.d90)}</div>
            </div>
          </div>
        </Card>
      )}
      {customers.map((c) => (
        <AccountRow key={c.id} customer={c} transactions={transactions} payments={payments} returns={returns} getBalance={getBalance} recordPayment={recordPayment} agingBuckets={agingBuckets} />
      ))}
    </>
  );
}

function TillTab({ transactions, returns, tillCounts, recordTillCount, currentStaff }) {
  const [date, setDate] = useState(todayISO());
  const [counted, setCounted] = useState('');
  const [note, setNote] = useState('');

  const expected = transactions.filter((t) => t.date === date && t.method === 'cash').reduce((s, t) => s + t.total, 0)
    - returns.filter((r) => r.date === date && r.originalMethod === 'cash').reduce((s, r) => s + r.refundAmount, 0);

  const todaysCounts = tillCounts.filter((tc) => tc.date === date);

  function submit() {
    const amt = parseFloat(counted);
    if (isNaN(amt)) { alert('Enter the counted cash amount.'); return; }
    recordTillCount({ date, counted: amt, note });
    setCounted(''); setNote('');
  }
  ...
  return (
    <>
      <Card>
        <Label>End-of-day cash count</Label>
        <div style={{ fontSize: '0.82rem', color: '#6b6659', marginBottom: 10 }}>
          Expected cash is today's cash/cheque sales minus cash refunds — everything already recorded in the till. Count the drawer and enter what's actually there; any difference is logged, not corrected away.
        </div>
        <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
          <input type="date" value={date} onChange={(e) => setDate(e.target.value)} style={{ padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: PARCHMENT, fontFamily: 'monospace' }} />
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontFamily: 'monospace', fontSize: '1.05rem', padding: '10px 0', borderTop: `1px solid ${LINE}`, borderBottom: `1px solid ${LINE}`, marginBottom: 10 }}>
          <span>Expected</span><span>{fmtMoney(expected)}</span>
        </div>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 8 }}>
          <input type="number" step="0.01" placeholder="Counted amount" value={counted} onChange={(e) => setCounted(e.target.value)} style={{ width: 140, padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, fontFamily: 'monospace', background: PARCHMENT }} />
          <input type="text" placeholder="Note (optional)" value={note} onChange={(e) => setNote(e.target.value)} style={{ flex: 1, minWidth: 140, maxWidth: 280, padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: PARCHMENT }} />
        </div>
        {counted !== '' && !isNaN(parseFloat(counted)) && (
          <div style={{ fontSize: '0.82rem', color: Math.abs(parseFloat(counted) - expected) < 0.005 ? OK : WARN, marginBottom: 10 }}>
            {Math.abs(parseFloat(counted) - expected) < 0.005
              ? 'Matches exactly.'
              : `${parseFloat(counted) > expected ? 'Over' : 'Short'} by ${fmtMoney(Math.abs(parseFloat(counted) - expected))}.`}
          </div>
        )}
        <button onClick={submit} style={{ background: GREEN, color: PARCHMENT, border: 'none', padding: '10px 18px', borderRadius: 3, fontWeight: 600, cursor: 'pointer' }}>
          Record count
        </button>
      </Card>
      ...
      {todaysCounts.length > 0 && (
        <Card>
          <Label>Counts recorded for {date}</Label>
          {todaysCounts.map((tc) => (
            <div key={tc.id} style={{ padding: '8px 0', borderBottom: `1px solid ${LINE}`, fontSize: '0.82rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span>{tc.time} · {tc.staff}</span>
                <span style={{ fontFamily: 'monospace', color: Math.abs(tc.variance) < 0.005 ? OK : WARN }}>
                  {fmtMoney(tc.counted)} ({tc.variance >= 0 ? '+' : ''}{fmtMoney(tc.variance)})
                </span>
              </div>
              {tc.note && <div style={{ fontSize: '0.76rem', color: '#8a8577', fontStyle: 'italic', marginTop: 2 }}>"{tc.note}"</div>}
            </div>
          ))}
        </Card>
      )}
      ...
      {tillCounts.length > 0 && (
        <Card>
          ...
          <div style={{ maxHeight: 260, overflowY: 'auto' }}>
            {tillCounts.slice(0, 20).map((tc) => (
              <div key={tc.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: `1px solid ${LINE}`, fontSize: '0.78rem', color: '#6b6659' }}>
                <span>{tc.date} · {tc.staff}</span>
                <span style={{ fontFamily: 'monospace', color: Math.abs(tc.variance) < 0.005 ? '#6b6659' : WARN }}>
                  expected {fmtMoney(tc.expected)} · counted {fmtMoney(tc.counted)}
                </span>
              </div>
            ))}
          </div>
    ...
    <>
      <div className="no-print">
        <Card>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <select value={customerId} onChange={(e) => setCustomerId(e.target.value)} style={{ flex: 1, minWidth: 200, maxWidth: 340, padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: PARCHMENT }}>
      ...
      <Card style={{ padding: '28px 26px' }}>
        ...
        <div style={{ marginBottom: 18 }}>
          ...
          {customer.terms && (
            <div style={{ fontSize: '0.78rem', color: BROWN, marginTop: 4, fontStyle: 'italic' }}>Payment terms: {customer.terms}</div>
          )}
        </div>
...
function CustomerRow({ customer, editCustomer, getBalance, setCustomerHold, setCustomerTerms }) {
  const [editing, setEditing] = useState(false);
  ...
  function save() {
    ...
    setCustomerTerms(customer.id, terms.trim());
    setEditing(false);
  }

  function placeHold() {
    if (!holdReasonInput.trim()) { alert('Enter a reason for the hold.'); return; }
    setCustomerHold(customer.id, true, holdReasonInput.trim());
    setShowHoldForm(false);
    setHoldReasonInput('');
  }

  function liftHold() {
    if (!confirm(`Lift the hold on ${customer.name}?`)) return;
    setCustomerHold(customer.id, false, `Hold lifted (was: ${customer.holdReason})`);
  }
  ...
  if (!editing) {
    return (
      <Card style={customer.onHold ? { borderColor: WARN } : undefined}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10, flexWrap: 'wrap' }}>
          <div>
            <div style={{ fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8 }}>
              {customer.name}
              {customer.onHold && (
                <span style={{ background: WARN, color: '#fff', fontFamily: 'monospace', fontSize: '0.62rem', fontWeight: 700, borderRadius: 10, padding: '2px 8px', textTransform: 'uppercase' }}>On hold</span>
              )}
            </div>
            <div style={{ fontSize: '0.76rem', color: '#8a8577' }}>{customer.location}</div>
            {customer.terms && (
              <div style={{ fontSize: '0.74rem', color: BROWN, marginTop: 2, fontStyle: 'italic' }}>Terms: {customer.terms}</div>
            )}
            {customer.onHold && customer.holdReason && (
              <div style={{ fontSize: '0.76rem', color: WARN, marginTop: 4 }}>"{customer.holdReason}"</div>
            )}
          </div>
          <div style={{ textAlign: 'right' }}>
        ...
        <div style={{ marginTop: 10, borderTop: `1px solid ${LINE}`, paddingTop: 10 }}>
          {customer.onHold ? (
            <button onClick={liftHold} style={{ background: 'none', border: `1px solid ${OK}`, color: OK, borderRadius: 3, padding: '6px 12px', fontSize: '0.76rem', fontWeight: 600, cursor: 'pointer' }}>
              Lift hold
            </button>
          ) : showHoldForm ? (
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              <input
                type="text" placeholder="Reason for hold…" value={holdReasonInput} onChange={(e) => setHoldReasonInput(e.target.value)}
                style={{ flex: 1, minWidth: 160, maxWidth: 280, padding: '7px 8px', border: `1px solid ${LINE}`, borderRadius: 3, fontSize: '0.8rem', background: PARCHMENT }}
              />
              <button onClick={placeHold} style={{ background: WARN, color: '#fff', border: 'none', padding: '7px 14px', borderRadius: 3, fontSize: '0.76rem', fontWeight: 600, cursor: 'pointer' }}>Confirm hold</button>
              <button onClick={() => setShowHoldForm(false)} style={{ background: 'none', border: `1px solid ${LINE}`, padding: '7px 14px', borderRadius: 3, fontSize: '0.76rem', cursor: 'pointer' }}>Cancel</button>
            </div>
          ) : (
            <button onClick={() => setShowHoldForm(true)} style={{ background: 'none', border: 'none', color: RUST, fontSize: '0.76rem', textDecoration: 'underline', cursor: 'pointer' }}>
              Put account on hold
            </button>
          )}
        </div>
    ...
    <Card>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxWidth: 420 }}>
        ...
        <input
          ...
          placeholder='Payment terms (e.g. "30 days" or "pay after harvest — due 30 Sep")'
          style={{ padding: '8px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: PARCHMENT }}
        />
        <div style={{ display: 'flex', gap: 8 }}>
...
function CustomersTab({ customers, addCustomer, editCustomer, getBalance, setCustomerHold, setCustomerTerms }) {
  const [name, setName] = useState('');
  ...
  return (
    <>
      ...
      <Card>
        <input type="text" placeholder={`Filter ${customers.length} customers…`} value={filter} onChange={(e) => setFilter(e.target.value)} style={{ width: '100%', maxWidth: 420, boxSizing: 'border-box', padding: '8px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: PARCHMENT }} />
      </Card>

      {filtered.map((c) => <CustomerRow key={c.id} customer={c} editCustomer={editCustomer} getBalance={getBalance} setCustomerHold={setCustomerHold} setCustomerTerms={setCustomerTerms} />)}
    </>
  );
}

function StaffRow({ staff, staffList, resetStaffPin }) {
  const [resetting, setResetting] = useState(false);
  const [newPin, setNewPin] = useState('');
  const [busy, setBusy] = useState(false);

  async function submitReset() {
    if (!newPin.trim()) { alert('Enter a new employee number.'); return; }
    setBusy(true);
    const ok = await resetStaffPin(staff.id, newPin);
    setBusy(false);
    if (ok) { setResetting(false); setNewPin(''); }
  }
  ...
  return (
    <div style={{ padding: '7px 0', borderBottom: `1px solid ${LINE}` }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '0.82rem' }}>
        <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ width: 10, height: 10, borderRadius: '50%', background: staffColor(staffList, staff.name), flexShrink: 0 }} />
          {staff.name}
        </span>
        <button onClick={() => setResetting((v) => !v)} style={{ background: 'none', border: 'none', color: RUST, fontSize: '0.74rem', textDecoration: 'underline', cursor: 'pointer' }}>
      ...
      {resetting && (
        <div style={{ display: 'flex', gap: 6, marginTop: 6, flexWrap: 'wrap' }}>
          <input
            type="text" placeholder="New employee number" value={newPin} onChange={(e) => setNewPin(e.target.value)}
            style={{ flex: 1, minWidth: 120, maxWidth: 200, padding: '6px 8px', border: `1px solid ${LINE}`, borderRadius: 3, fontFamily: 'monospace', fontSize: '0.8rem', background: PARCHMENT }}
          />
          <button onClick={submitReset} disabled={busy} style={{ background: GREEN_MID, color: PARCHMENT, border: 'none', padding: '6px 12px', borderRadius: 3, fontSize: '0.76rem', fontWeight: 600, cursor: 'pointer' }}>
...
function SettingsTab({ staffList, addStaffMember, resetStaffPin, corrections, exportAllData, lockAfterMinutes, setLockAfterMinutes }) {
  ...
  async function submit() {
    ...
    setAdding(true);
    await addStaffMember(newStaffName, newStaffPin);
    setAdding(false);
  ...
  return (
    <>
      ...
      <Card>
        <Label>Staff</Label>
        <div style={{ fontSize: '0.82rem', color: '#6b6659', marginBottom: 10 }}>
          Each staff member gets their own employee number. Whoever unlocks the screen with it is attached to every sale, delivery, payment, and correction from that point on. Numbers are hashed, not stored as plain text — nobody, including this screen, can look an existing one back up. Use "Reset PIN" if someone forgets theirs.
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 0, marginBottom: 10 }}>
          {staffList.map((s) => (
            <StaffRow key={s.id} staff={s} staffList={staffList} resetStaffPin={resetStaffPin} />
          ))}
        </div>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <input type="text" placeholder="Staff name" value={newStaffName} onChange={(e) => setNewStaffName(e.target.value)} style={{ flex: 1, minWidth: 120, maxWidth: 280, padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: PARCHMENT }} />
          ...
          <button onClick={submit} disabled={adding} style={{ background: GREEN_MID, color: PARCHMENT, border: 'none', padding: '9px 16px', borderRadius: 3, fontWeight: 600, cursor: 'pointer' }}>{adding ? 'Adding…' : 'Add'}</button>
        </div>
      </Card>
...
function LockScreen({ tryUnlock }) {
  ...
  return (
    <div style={{
      ...
      />
      {lockout.isLocked ? (
        <div style={{ fontSize: '0.78rem', color: '#f0a98f', marginBottom: 12 }}>Too many wrong attempts — try again in {lockout.remainingSeconds}s.</div>
      ) : error ? (
        <div style={{ fontSize: '0.78rem', color: '#f0a98f', marginBottom: 12 }}>{error}{lockout.attemptsLeft <= 2 ? ` (${lockout.attemptsLeft} attempt${lockout.attemptsLeft === 1 ? '' : 's'} left)` : ''}</div>
      ) : null}
      <button
        onClick={submit}
        disabled={lockout.isLocked || checking}
