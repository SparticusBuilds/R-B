    ...
    <div style={{ fontFamily: 'Inter, sans-serif', background: PARCHMENT, minHeight: '100vh', color: CHARCOAL, fontSize: 14 }}>
      ...
      <main style={
        ...
        {!loaded ? (
          ...
          <SalesTab products={products} customers={customers} stockFor={stockFor} recordSale={recordSale} staffList={staffList} currentStaff={currentStaff} />
        ) : tab === 'quote' ? (
          <QuoteTab products={products} customers={customers} stockFor={stockFor} recordSale={recordSale} quotes={quotes} saveQuote={saveQuote} markQuoteConverted={markQuoteConverted} staffList={staffList} currentStaff={currentStaff} />
        ) : tab === 'goodsin' ? (
          ...
          <SpecialOrdersTab
            products={products} customers={customers} stockFor={stockFor}
...
function SalesTab({ products, customers, stockFor, recordSale, staffList, currentStaff }) {
  ...
  return (
    <>
      <Card>
        ...
        <select value={customerId} onChange={(e) => { setCustomerId(e.target.value); setHoldOverrideBy(''); }} style={{ width: '100%', padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: PARCHMENT, fontSize: '0.9rem' }}>
...
function QuoteTab({ products, customers, stockFor, recordSale, quotes, saveQuote, markQuoteConverted, staffList, currentStaff }) {
  ...
  return (
    <>
      ...
      {view === 'history' ? (
        <QuoteHistory
          quotes={quotes} products={products} customers={customers} stockFor={stockFor}
...
function QuoteHistory({ quotes, products, customers, stockFor, recordSale, markQuoteConverted, staffList, currentStaff }) {
      ...
      {filtered.map((q) => (
        <Card key={q.id}>
          ...
          {q.converted ? (
            ...
            <ConvertQuoteForm
              quote={q} products={products} customers={customers} stockFor={stockFor} recordSale={recordSale}
              staffList={staffList} currentStaff={currentStaff}
...
function ConvertQuoteForm({ quote, products, customers, stockFor, recordSale, staffList, currentStaff, onDone, onCancel }) {
  ...
  const [lines, setLines] = useState(() => quote.items.map((it) => {
    ...
    return { ...it, available, qty: Math.min(it.qty, Math.max(0, available)) };
  }));
  ...
  function finalize(pomAssessment, holdOverride) {
    const usable = lines.filter((l) => l.qty > 0).map((l) => ({ productId: l.productId, name: l.name, price: l.price, qty: l.qty, discount: 0 }));
  ...
  return (
    <div style={{ marginTop: 10, background: PARCHMENT, border: `1px solid ${LINE}`, borderRadius: 3, padding: 12 }}>
      ...
      <select value={customerId} onChange={(e) => { setCustomerId(e.target.value); setHoldOverrideBy(''); }} style={{ width: '100%', padding: '8px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: '#fff', marginBottom: 8 }}>
        {customers.map((c) => <option key={c.id} value={c.id}>{c.name} — {c.location}{c.onHold ? ' — ON HOLD' : ''}</option>)}
      </select>
      ...
      {lines.map((l) => {
        ...
        return (
          <div key={l.productId} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', gap: 8 }}>
...
function NewSpecialOrderForm({ products, customers, createSpecialOrder, onDone }) {
  ...
  return (
    <>
      <Card>
        <Label>Customer</Label>
        <select value={customerId} onChange={(e) => setCustomerId(e.target.value)} style={{ width: '100%', padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: PARCHMENT }}>
      ...
      {cart.length > 0 && (
        <Card>
          ...
          <input type="text" placeholder="Notes (optional — e.g. expected delivery)" value={notes} onChange={(e) => setNotes(e.target.value)} style={{ width: '100%', marginTop: 8, padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: PARCHMENT }} />
...
function FulfillSpecialOrderForm({ order, products, stockFor, staffList, currentStaff, fulfillSpecialOrder, onDone, onCancel }) {
  const [method, setMethod] = useState('account');
...
function SpecialOrdersTab({ products, customers, stockFor, recordSale, specialOrders, createSpecialOrder, fulfillSpecialOrder, cancelSpecialOrder, staffList, currentStaff }) {
  const [view, setView] = useState('new'); // 'new' | 'history'
  const [saved, setSaved] = useState(null);
  const [search, setSearch] = useState('');
              ...
              <Card key={o.id}>
                ...
                {o.status === 'fulfilled' ? (
                  ...
                  <FulfillSpecialOrderForm
                    order={o} products={products} stockFor={stockFor} staffList={staffList} currentStaff={currentStaff}
    ...
    <div style={{ background: PARCHMENT, border: `1px solid ${LINE}`, borderRadius: 3, padding: 10, margin: '4px 0' }}>
      ...
      {isHazardous && (
        <input
          ...
          style={{ width: '100%', padding: '6px 8px', border: `1px solid ${LINE}`, borderRadius: 3, background: '#fff', marginBottom: 8 }}
        />
      )}
      <input type="text" placeholder="Reason for correction (required)" value={reason} onChange={(e) => setReason(e.target.value)} style={{ width: '100%', padding: '6px 8px', border: `1px solid ${LINE}`, borderRadius: 3, background: '#fff', marginBottom: 8, fontSize: '0.8rem' }} />
      <div style={{ display: 'flex', gap: 8 }}>
    ...
    <>
      <Card>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 8 }}>
          ...
          <input type="text" placeholder="Note (optional)" value={note} onChange={(e) => setNote(e.target.value)} style={{ flex: 1, minWidth: 140, padding: '9px 10px', border: `1px solid ${LINE}`, borderRadius: 3, background: PARCHMENT }} />
        </div>
      ...
      {resetting && (
        <div style={{ display: 'flex', gap: 6, marginTop: 6, flexWrap: 'wrap' }}>
          <input
            ...
            style={{ flex: 1, minWidth: 120, padding: '6px 8px', border: `1px solid ${LINE}`, borderRadius: 3, fontFamily: 'monospace', fontSize: '0.8rem', background: PARCHMENT }}
          />
