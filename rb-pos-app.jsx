...
export default function RBPos() {
  ...
  function recordSale({ customerId, items, method, discountApprovedBy }) {
    ...
    const tx = {
      id: uid('tx'), ref: nextSaleRef(), date: todayISO(), customerId, items: lineDetails, total, method, staff: currentStaff,
          ...
          <Card key={tx.id}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
              ...
              <div style={{ textAlign: 'right' }}>
                ...
                <div style={{ fontFamily: 'monospace', fontSize: '0.74rem', color: '#8a8577' }}>{tx.date} · {tx.method === 'account' ? 'Account' : 'Cash/cheque'}</div>
              </div>
            </div>
            {tx.items.map((item, i) => (
